    ...
    F: FnMut(&ISimpleAudioVolume) -> Result<()>,
{
    ...
    let mut candidates: Vec<(String, ISimpleAudioVolume)> = Vec::new();
    ...
    unsafe {
        ...
        for i in 0..sessions.GetCount()? {
            ...
            candidates.push((stem.to_string(), ctrl2.cast()?));
        }

        // 有精确同名的就只用它们，否则退回全部子串命中项。
        let exact = candidates.iter().any(|(name, _)| name == needle);

        for (name, vol) in &candidates {
