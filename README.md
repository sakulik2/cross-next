...
首次运行在 exe 同目录生成 `config.json`，并打印访问地址：
...
- **改 `remote.json` 即时生效**，不必重启。换了服务端地址或 token 直接存盘就行。
  刻意不重启进程 —— 重启要先释放热键再重新注册，那个空隙里别的程序可能抢走。
  文件读坏或 token 被清空时保留原配置继续跑，不会因为一次误编辑就失效。

### 服务端连不上时

太久没按键（5 分钟）就试一次 `/api/state` 确认服务端还在；按键本身就是连通性测试，
刚按过不会白探。连续 5 次失败就**放开媒体键**，让它们回到本机播放器，同时后台按
5/10/20/40/60 秒退避重连，服务端回来后自动抢回。

所以台式机休眠期间媒体键会控制本机播放器，唤醒后自动恢复 —— 这段时间按键「失灵」
是预期行为，不是出错。进程刻意不退出：连不上恰恰是最需要它待命的时刻，而且没有界面，
自己退出了用户既看不见也没法重启。

抢回可能失败（让出期间被别的程序占了），下一轮探活还会再试。从命令行启动时这些状态
变化都会打印。

启动报「媒体键全部注册失败」表示这些键被 cross-next 之外的程序占用 —— 常见的是播放器的
全局热键设置、键盘厂商驱动、其它媒体控制小工具。Windows 不提供反查热键占用者的接口。
...
- `canSeek` — 应用自报是否允许拖动定位。**前端刻意不据此禁用滑杆**：QQ音乐 报
  false 却照样接受 `TryChangePlaybackPositionAsync` 并真的跳过去。和播放键同样处理 ——
  命令照发，被拒绝就如实报出来。
